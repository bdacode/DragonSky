var Enemy = cc.Sprite.extend({
    ...
    active:true,
    speed:200,
    bulletSpeed:DS.BULLET_SPEED.ENEMY,
    HP:15,
    bulletPowerValue:1,
    moveType:null,
    scoreValue:200,
    ...
    shoot:function () {
        var p = this.getPosition();
        var b = Bullet.getOrCreateBullet(this.bulletSpeed, "W2.png", this.attackMode, 3000, DS.UNIT_TAG.ENMEY_BULLET);
...
Enemy.getOrCreateEnemy = function (arg) {
    var selChild = null;
    for (var j = 0; j < DS.CONTAINER.ENEMIES.length; j++) {
        selChild = DS.CONTAINER.ENEMIES[j];
...
Enemy.create = function (arg) {
    ...
    g_sharedGameLayer.addEnemy(enemy, enemy.zOrder, DS.UNIT_TAG.ENEMY);
    DS.CONTAINER.ENEMIES.push(enemy);
    return enemy;
};
